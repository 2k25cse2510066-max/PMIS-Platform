        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            {onToggleSidebar && (
              <button
                ...
                className="p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-white/10 lg:hidden"
          ...
          <div className="flex-1 max-w-md hidden md:block">
            <div className="relative flex items-center">
              <svg className="w-4 h-4 absolute left-3.5 text-slate-400 dark:text-slate-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <input
                ...
                className="input pl-9 pr-9 text-xs !bg-white/80 dark:!bg-[#151733] dark:!border-[#26294D] !text-slate-900 dark:!text-white placeholder:!text-slate-400 dark:placeholder:!text-slate-400 !rounded-xl"
              />
              <span className="absolute right-3 px-1.5 py-0.5 rounded text-[10px] font-mono font-bold bg-slate-200/80 dark:bg-white/10 text-slate-500 dark:text-slate-400 border border-slate-300 dark:border-white/10 pointer-events-none">
                /
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {user && (
              <div className="relative" ref={notifRef}>
                <button
                  ...
                  className="relative p-2 rounded-xl text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-white/10 transition-all"
                  title="Notifications"
                >
                  ...
                  <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-3xl bg-white/95 dark:bg-[#0F0D25]/95 backdrop-blur-2xl border border-slate-200/90 dark:border-white/15 p-4 shadow-2xl z-50 space-y-3">
                    <div className="font-display font-bold text-sm text-slate-900 dark:text-white border-b border-slate-200 dark:border-white/10 pb-2 flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        ...
                        {unreadCount > 0 && (
                          <span className="bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 text-[11px] font-mono px-2 py-0.5 rounded-full font-bold">
                            {unreadCount} new
                          </span>
                        )}
                      </div>
                      {notifications.length > 0 && (
                        <button
                          ...
                          className="text-[11px] text-indigo-600 dark:text-indigo-300 font-semibold hover:underline"
                        >
                          Mark all as read
                        </button>
                      )}
                    ...
                    <div className="max-h-72 overflow-y-auto space-y-2.5 pr-1">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          <div
                            key={n.id}
                            className="p-3 rounded-2xl bg-slate-100/90 dark:bg-white/[0.06] border border-slate-200/90 dark:border-white/10 text-xs text-slate-800 dark:text-slate-200 space-y-1 transition-all hover:bg-slate-200/70 dark:hover:bg-white/[0.12]"
                          >
                            <p className="leading-snug font-medium text-slate-800 dark:text-slate-100">{n.message}</p>
            ...
            <button
              ...
              className="p-2 rounded-xl text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-white hover:bg-slate-200/60 dark:hover:bg-white/10 transition-all"
            ...
            <div className="relative" ref={settingsRef}>
              ...
              {/* Settings Dropdown */}
              {showSettings && (
                <div className="absolute right-0 mt-2 w-72 sm:w-80 rounded-3xl bg-white/95 dark:bg-[#0F0D25]/95 backdrop-blur-2xl border border-slate-200/90 dark:border-white/15 p-4 shadow-2xl z-50 space-y-4">
                  <div className="font-display font-bold text-sm text-slate-900 dark:text-white border-b border-slate-200 dark:border-white/10 pb-2 flex justify-between items-center">
                  ...
                  <div className="space-y-2 border-t border-slate-200 dark:border-white/10 pt-3">
                    ...
                    <div className="space-y-1 text-xs">
                      <button
                        ...
                        className="w-full text-left py-1.5 px-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 text-slate-800 dark:text-slate-200 font-semibold flex items-center justify-between"
                      >
                      ...
                      >
                        ...
                        <span className="text-[10px] text-indigo-600 dark:text-indigo-300 font-mono bg-indigo-500/15 px-1.5 py-0.5 rounded font-bold">
                          Managed
                        </span>
                      </button>
                    </div>
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <div className="hidden lg:flex flex-col text-right leading-tight">
                  ...
                  <span className="chip text-[10px] capitalize self-end py-0 px-1.5 mt-0.5 font-bold">
                    {user.role}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setShowPasswordModal(false)}>
          <div className="glass-panel max-w-md w-full p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/10 pb-3">
              <h3 className="font-display font-bold text-lg text-slate-900 dark:text-white">Change Password</h3>
              <button onClick={() => setShowPasswordModal(false)} className="text-slate-400 hover:text-slate-700 dark:hover:text-white text-xl font-bold">×</button>
            </div>
            <div className="p-3 bg-slate-100 dark:bg-white/[0.05] rounded-2xl text-xs text-slate-700 dark:text-slate-300 space-y-1 border border-slate-200 dark:border-white/10">
              <p className="font-bold text-slate-900 dark:text-white">Security Note:</p>
