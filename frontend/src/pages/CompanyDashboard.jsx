          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-200 text-xs mt-1 font-medium">Post internship seats, review candidate match breakdown, and manage status hiring pipelines.</p>
              </div>
            </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {filteredApps.length === 0 ? (
        ...
        filteredApps.map((a) => (
          <div key={a.application_id} className="glass-card p-4 flex items-center gap-4 border-slate-200/90 dark:border-white/15 cursor-pointer hover:border-indigo-500/50" onClick={() => setSelectedApplicant(a)}>
            <MatchSeal score={a.match.overall} size={52} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-bold text-slate-900 dark:text-white text-sm hover:underline">{a.name}</span>
                {a.resume_filename && (
                  <span className="text-[10px] bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 px-2 py-0.5 rounded-full font-mono border border-indigo-500/30 font-bold">📄 Resume attached</span>
                )}
              </div>
              <div className="text-xs text-slate-600 dark:text-slate-300 mt-0.5 font-medium">{a.location || 'Location not set'} {a.cgpa ? `· CGPA ${a.cgpa}` : ''} {a.phone ? `· ${a.phone}` : ''}</div>
            ...
            <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
              <select
                ...
                className="input !w-auto text-xs capitalize bg-white text-slate-900 dark:bg-[#0C0A1D] dark:text-white"
              >
      ...
      {selectedApplicant && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setSelectedApplicant(null)}>
          <div className="glass-panel max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto space-y-4 border-slate-200/90 dark:border-white/20" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between border-b border-slate-200 dark:border-white/10 pb-3">
              <div>
                <h3 className="font-display text-2xl font-bold text-slate-900 dark:text-white">{selectedApplicant.name}</h3>
            ...
            <div className="flex items-center gap-4 bg-slate-100/90 dark:bg-white/[0.05] p-3.5 rounded-2xl border border-slate-200 dark:border-white/10">
              <MatchSeal score={selectedApplicant.match.overall} size={56} />
              <div>
                <div className="font-display font-bold text-slate-900 dark:text-white text-sm">Match Score: {selectedApplicant.match.overall}%</div>
            ...
            {selectedApplicant.resume_filename ? (
              <div className="flex items-center justify-between bg-emerald-500/15 border border-emerald-500/30 p-3 rounded-2xl">
                ...
                {selectedApplicant.resume_url ? (
                  ...
                  <span className="text-xs text-emerald-700 dark:text-emerald-400 font-semibold">Resume link unavailable</span>
                )}
              </div>
            ) : (
            ...
            <div>
              <h4 className="font-display text-xs font-bold text-slate-900 dark:text-white mb-1">Skills</h4>
